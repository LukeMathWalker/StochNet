...
def build_simulation_dataset(model_name, nb_settings, nb_trajectories,
                             timestep, endtime, dataset_folder,
                             algorithm, prefix='partial_', how='concat'):
    perform_simulations(model_name, nb_settings, nb_trajectories, timestep,
                        endtime, dataset_folder, algorithm, prefix=prefix)
    if how == 'concat':
        dataset = concatenate_simulations(nb_settings,
                                          dataset_folder,
                                          prefix=prefix)
    elif how == 'stack':
...
def perform_simulations(model_name, nb_settings, nb_trajectories,
                        timestep, endtime, dataset_folder,
                        algorithm, prefix='partial_'):
    # parallel for cycle
    program_module = import_module(
        "stochnet.dataset.single_simulation_w_gillespy"
    )
...
def concatenate_simulations(nb_settings, dataset_folder, prefix='partial_'):
    ...
    for i in tqdm(range(nb_settings)):
        ...
        if i == 0:
            ...
            final_dataset = np.concatenate((final_dataset, partial_dataset),
                                           axis=0)
...
if __name__ == '__main__':
    ...
    dataset = build_simulation_dataset(model_name, nb_settings,
                                       nb_trajectories, timestep,
                                       endtime,
    ...
    with open(dataset_explorer.log_fp, 'a') as f:
        f.write("Simulating {0} {1} trajectories for {2} "
                "different settings using {3} (with endtime "
