...
def set_initial_parameters(simulation_obj, setting_collection, current_index):
    S = int(setting_collection[current_index, 0])
    I = int(setting_collection[current_index, 1])
    R = int(setting_collection[current_index, 2])
...
for i in tqdm(range(nb_of_settings)):
    ...
    smod.DoStochSim(method="direct", trajectories=trajectories_nb, mode="time", end=time_step_for_resampling)
...
with open('SIR_dataset_timestep_2-5_03.npy', 'wb') as dataset_filepath:
